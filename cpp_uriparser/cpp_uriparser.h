...
namespace core
{
  namespace common_parsers
  {
    ...
    public:
      ...
      IteratorType operator++(int) const // NOLINT (c-style cast)
      {
    ...
    {
      typedef std::function<int(UriStateType*, UrlTextType)> ParseUriProc; // NOLINT
      typedef std::function<void(UriObjType*)> FreeUriMembersProc; // NOLINT
      typedef std::function<int(UriObjType*)> UriNormalizeSyntaxProc; // NOLINT
